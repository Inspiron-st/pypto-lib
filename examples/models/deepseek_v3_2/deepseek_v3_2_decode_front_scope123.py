...
def build_deepseek_v3_2_decode_front_scope123_program(
    ...
    class DeepSeekV32DecodeFrontScope123:
        ...
        def deepseek_v3_2_decode_front_scope123(
            ...
            norm_pack: pl.Tensor[[1, HIDDEN_CFG + Q_LORA_RANK_CFG + KV_LORA_RANK_CFG], pl.FP32],
            ...
            k_norm_affine: pl.Tensor[[2, INDEX_HEAD_DIM_CFG], pl.FP32],
            ...
            input_rms_weight = pl.slice(norm_pack, [1, HIDDEN_CFG], [0, 0])
            q_norm_weight = pl.slice(norm_pack, [1, Q_LORA_RANK_CFG], [0, HIDDEN_CFG])
            kv_norm_weight = pl.slice(norm_pack, [1, KV_LORA_RANK_CFG], [0, HIDDEN_CFG + Q_LORA_RANK_CFG])
            k_norm_weight = pl.slice(k_norm_affine, [1, INDEX_HEAD_DIM_CFG], [0, 0])
            k_norm_bias = pl.slice(k_norm_affine, [1, INDEX_HEAD_DIM_CFG], [1, 0])
            ...
            for b0 in pl.range(0, BATCH_CFG, BATCH_TILE):
                ...
                for h in pl.parallel(0, NUM_HEADS_CFG, 1):
                    ...
                    q_pe_head = pl.slice(q_pe_out, [BATCH_TILE, QK_ROPE_HEAD_DIM_CFG], [b0, q_pe_col])
                    with pl.at(level=pl.Level.CORE_GROUP):
                        for b in pl.range(BATCH_TILE):
                            ...
                            sin_lo = pl.slice(
                                rope_pair, [1, QK_ROPE_HEAD_DIM_CFG // 2], [MAX_SEQ_CFG + pos, 0]
                            )
                            sin_hi = pl.slice(
                                rope_pair,
                            ...
                            q_lo = pl.cast(pl.slice(q_pe_head, [1, QK_ROPE_HEAD_DIM_CFG // 2], [b, 0]), target_type=pl.FP32)
                            q_hi = pl.cast(
                                pl.slice(q_pe_head, [1, QK_ROPE_HEAD_DIM_CFG // 2], [b, QK_ROPE_HEAD_DIM_CFG // 2]),
                                target_type=pl.FP32,
                            )
                            ...
                            q_pe_head = pl.assemble(q_pe_head, pl.cast(q_rot_lo, target_type=pl.BF16), [b, 0])
                            q_pe_head = pl.assemble(
                                q_pe_head,
                                pl.cast(q_rot_hi, target_type=pl.BF16),
                                [b, QK_ROPE_HEAD_DIM_CFG // 2],
                            )
                    q_pe_out = pl.assemble(q_pe_out, q_pe_head, [b0, q_pe_col])
            ...
            with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.auto_chunk]):
                for ob in pl.parallel(0, IDX_OUT_BLOCKS, 1, chunk=8):
                    ...
                    for kb in pl.range(QR_BLOCKS):
                        k0 = kb * LORA_CHUNK
                        qr_chunk = pl.slice(qr_out, [BATCH_TILE, LORA_CHUNK], [0, k0])
            ...
            with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.auto_chunk]):
                for ob in pl.parallel(0, WK_OUT_BLOCKS, 1, chunk=1):
                    ...
                    for kb in pl.range(HIDDEN_BLOCKS):
                        k0 = kb * K_CHUNK
                        s2_x_chunk = pl.slice(hidden_states, [BATCH_TILE, K_CHUNK], [0, k0])
            ...
            with pl.at(level=pl.Level.CORE_GROUP):
                s2_k_tile = pl.cast(pl.slice(k_idx, [BATCH_TILE, INDEX_HEAD_DIM_CFG], [0, 0]), target_type=pl.FP32)
                ...
                k_idx = pl.assemble(k_idx, pl.cast(s2_k_normed, target_type=pl.BF16), [0, 0])
            ...
            with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.auto_chunk]):
                for ob in pl.parallel(0, IDX_OUT_BLOCKS, 1, chunk=8):
                    q0 = ob * IDX_OUT_CHUNK
                    s2_q_tile = pl.cast(pl.slice(q_idx_full, [BATCH_TILE, IDX_OUT_CHUNK], [0, q0]), target_type=pl.FP32)
                    s2_q_scaled = pl.mul(s2_q_tile, HADAMARD_SCALE)
                    q_idx_full = pl.assemble(q_idx_full, pl.cast(s2_q_scaled, target_type=pl.BF16), [0, q0])

                s2_k_tile = pl.cast(pl.slice(k_idx, [BATCH_TILE, INDEX_HEAD_DIM_CFG], [0, 0]), target_type=pl.FP32)
                s2_k_scaled = pl.mul(s2_k_tile, HADAMARD_SCALE)
                k_idx = pl.assemble(k_idx, pl.cast(s2_k_scaled, target_type=pl.BF16), [0, 0])
            ...
            with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.auto_chunk]):
                for ob in pl.parallel(0, WEIGHTS_BLOCKS, 1, chunk=1):
                    ...
                    for kb in pl.range(HIDDEN_BLOCKS):
                        ...
                        s2_w_acc = pl.add(s2_w_acc, pl.matmul(s2_x_chunk, pl.cast(wp_chunk, target_type=pl.BF16), out_dtype=pl.FP32))
...
def golden_decode_front_scope123(tensors):
    ...
    norm_pack = tensors["norm_pack"].float()
    input_rms_weight = norm_pack[:, :HIDDEN]
    q_norm_weight = norm_pack[:, HIDDEN : HIDDEN + Q_LORA_RANK]
    kv_norm_weight = norm_pack[:, HIDDEN + Q_LORA_RANK :]
    ...
    k_norm_affine = tensors["k_norm_affine"].float()
    k_norm_weight = k_norm_affine[0:1, :]
    k_norm_bias = k_norm_affine[1:2, :]
    ...
    q_nope = q_proj_view[:, :, :QK_NOPE_HEAD_DIM].reshape(q_proj.shape[0], -1).to(torch.bfloat16)
    q_pe = q_proj_view[:, :, QK_NOPE_HEAD_DIM:].reshape(q_proj.shape[0], -1).to(torch.bfloat16)
    del q_nope, q_pe
    ...
    # NOTE: fused scope123 currently skips strict idx equality check.
    # Keep runtime-produced topk_idx_out to avoid false negatives from
    # ordering differences in equal/near-equal score regions.
...
def build_tensor_specs(
    ...
    def init_norm_pack():
        return torch.cat([init_rms_weight(), init_q_norm_weight(), init_kv_norm_weight()], dim=1)
    ...
    def init_k_norm_affine():
        return torch.cat([init_k_norm_weight(), init_k_norm_bias()], dim=0)
    ...
    return [
        ...
        TensorSpec(
            "norm_pack",
            [1, hidden_size + q_lora_rank + kv_lora_rank],
            torch.float32,
            init_value=init_norm_pack,
        ),
        ...
        TensorSpec("k_norm_affine", [2, index_head_dim], torch.float32, init_value=init_k_norm_affine),
