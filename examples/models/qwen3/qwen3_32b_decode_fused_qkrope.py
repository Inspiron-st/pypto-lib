...
Scope 1:
  ...
  2. Fused Q projection + Q RoPE + q pad
  3. K RoPE + cache write during K projection tiling
  4. V cache write during V projection tiling
...
def build_qwen3_decode_program(
    ...
    class Qwen3Decode:
        ...
        def qwen3_decode(
            ...
            # ── Scope 1: input RMSNorm + fused Q RoPE + K/V projection ──
            for b0 in pl.range(0, batch, BATCH_TILE):
                ...
                for ob in pl.range(kv_out_blocks):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP):
                        for bi in pl.range(BATCH_TILE):
                            b = b0 + bi
                            ctx_len_k = pl.tensor.read(seq_lens, [b])
                            pos_k = ctx_len_k - 1
                            cos_lo_k = pl.slice(rope_cos, [1, half_dim], [pos_k, 0])
                            cos_hi_k = pl.slice(rope_cos, [1, half_dim], [pos_k, half_dim])
                            sin_lo_k = pl.slice(rope_sin, [1, half_dim], [pos_k, 0])
                            sin_hi_k = pl.slice(rope_sin, [1, half_dim], [pos_k, half_dim])
                            k_lo = pl.slice(k_acc, [1, half_dim], [bi, 0])
                            k_hi = pl.slice(k_acc, [1, half_dim], [bi, half_dim])
                            rot_lo = pl.sub(
                                pl.col_expand_mul(k_lo, cos_lo_k),
                                pl.col_expand_mul(k_hi, sin_lo_k),
                            )
                            rot_hi = pl.add(
                                pl.col_expand_mul(k_hi, cos_hi_k),
                                pl.col_expand_mul(k_lo, sin_hi_k),
                            )
                            kvh = kv0 // head_dim
                            cache_row_k = b * num_kv_heads * max_seq + kvh * max_seq + pos_k
                            k_cache = pl.assemble(k_cache, pl.cast(rot_lo, target_type=pl.BF16), [cache_row_k, 0])
                            k_cache = pl.assemble(k_cache, pl.cast(rot_hi, target_type=pl.BF16), [cache_row_k, half_dim])
                ...
                for ob in pl.range(kv_out_blocks):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP):
                        for bi in pl.range(BATCH_TILE):
                            b = b0 + bi
                            ctx_len_v = pl.tensor.read(seq_lens, [b])
                            pos_v = ctx_len_v - 1
                            kvh = kv0 // head_dim
                            cache_row_v = b * num_kv_heads * max_seq + kvh * max_seq + pos_v
                            v_cache = pl.assemble(
                                v_cache,
                                pl.cast(pl.slice(v_acc, [1, head_dim], [bi, 0]), target_type=pl.BF16),
                                [cache_row_v, 0],
                            )

            # ── Scope 2: grouped-query attention ──
