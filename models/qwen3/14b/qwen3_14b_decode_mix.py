...
def _require_divisible(value_name: str, value: int, chunk_name: str, chunk: int):
    if value % chunk != 0:
        raise ValueError(f"{value_name} must be divisible by {chunk_name}")
...
def _validate_decode_config(
    ...
    _require_divisible("hidden_size", hidden_size, "INPUT_PROJ_K_CHUNK", INPUT_PROJ_K_CHUNK)
    _require_divisible("hidden_size", hidden_size, "KV_PROJ_K_CHUNK", KV_PROJ_K_CHUNK)
    _require_divisible("hidden_size", hidden_size, "OUT_PROJ_K_CHUNK", OUT_PROJ_K_CHUNK)
    _require_divisible("hidden_size", hidden_size, "K_CHUNK", K_CHUNK)
    _require_divisible("hidden_size", hidden_size, "DOWN_OUT_CHUNK", DOWN_OUT_CHUNK)
    _require_divisible("hidden_size", hidden_size, "OUT_PROJ_N_CHUNK", OUT_PROJ_N_CHUNK)
    _require_divisible("intermediate_size", intermediate_size, "MLP_OUT_CHUNK", MLP_OUT_CHUNK)
    _require_divisible("intermediate_size", intermediate_size, "DOWN_MLP_CHUNK", DOWN_MLP_CHUNK)

    q_per_kv = num_heads // num_kv_heads
    if q_per_kv % Q_HEAD_BATCH != 0:
        raise ValueError("q_per_kv must be divisible by Q_HEAD_BATCH")
...
def build_qwen3_decode_program(
    ...
    # The `batch` parameter defines the static in-kernel capacity for
    # scratch tensors. Every batch-dependent kernel signature dim is a
    # pl.dynamic() variable (USER_BATCH_DYN / BLOCK_TABLE_FLAT_DYN /
    # KV_CACHE_ROWS_DYN), so a single compiled program serves any
    # user_batch <= that capacity.
    _validate_decode_config(
    ...
    batch_capacity_padded = ((batch + BATCH_TILE - 1) // BATCH_TILE) * BATCH_TILE
    ...
    class Qwen3Decode:
        ...
        def qwen3_decode(
            ...
            # Allocated at compile-time batch_capacity_padded; pl.create_tensor
            # zero-inits so trailing (batch_padded - user_batch) padded rows are 0,
            # which is the invariant relied on by Q/K-norm and scope-3.
            q_proj = pl.create_tensor([batch_capacity_padded, hidden], dtype=pl.FP32)
            k_proj = pl.create_tensor([batch_capacity_padded, kv_hidden], dtype=pl.FP32)
            v_proj = pl.create_tensor([batch_capacity_padded, kv_hidden], dtype=pl.FP32)
            q_proj_norm = pl.create_tensor([batch_capacity_padded, hidden], dtype=pl.FP32)
            k_proj_norm = pl.create_tensor([batch_capacity_padded, kv_hidden], dtype=pl.FP32)
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_norm"):
                    for h in pl.parallel(num_kv_heads):
                        for qg in pl.parallel(q_groups):
                            q0 = (h * q_per_kv + qg * Q_HEAD_BATCH) * head_dim
                            q_chunk = pl.reshape(
            ...
            # attn_out is allocated at batch_capacity_padded so scope-3 (which
            # loops over runtime batch_padded) can read full BATCH_TILE rows in every
            ...
            attn_out = pl.create_tensor([batch_capacity_padded, hidden], dtype=pl.BF16)
            all_q_padded = pl.create_tensor(
                [batch_capacity_padded * total_q_groups * Q_HEAD_PAD, head_dim], dtype=pl.BF16,
            )

            # Outer loop iterates user_batch sequentially (one row per iter).
            # seq_lens is clamped to [1, max_seq] before any RoPE/cache access
            # so pos never underflows and block_table reads stay within the
            # per-sequence max_blocks_per_seq capacity. Padded b rows do not
            # need attention, so their zero-initialized attn_out rows remain unchanged.
            for b in pl.parallel(user_batch):
                ctx_len_raw = pl.tensor.read(seq_lens, [b])
                min_ctx_len = pl.cast(1, pl.INT32)
                max_ctx_len = pl.cast(max_seq, pl.INT32)
                ctx_len = ctx_len_raw
                ...
                slot_logical_block = pos // BLOCK_SIZE
                slot_offset = pos - slot_logical_block * BLOCK_SIZE
                slot_block = pl.cast(
                    pl.tensor.read(block_table, [block_table_base + slot_logical_block]),
                    pl.INDEX,
                )
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="rope_kv_cache"):
                    for ki in pl.parallel(num_kv_heads):
                        kv_col = ki * head_dim
                        ...
                        for qg in pl.parallel(q_groups):
                            q_base = ki * q_per_kv + qg * Q_HEAD_BATCH
                            q_group = ki * q_groups + qg
                            q_block = pl.reshape(
                            ...
                            q_padded_row = b * total_q_groups * Q_HEAD_PAD + q_group * Q_HEAD_PAD
                ...
                for gi in pl.parallel(total_q_groups):
                    ...
                    q_padded = all_q_padded[q_padded_row : q_padded_row + Q_HEAD_PAD, :]
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                for ob in pl.parallel(mlp_out_blocks):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.auto_chunk], name_hint="silu"):
...
def golden_qwen3_decode(tensors):
    ...
    for b in range(batch):
        ctx_len = max(1, min(int(seq_lens[b].item()), max_seq))
        pos = ctx_len - 1
        ctx_blocks = (ctx_len + BLOCK_SIZE - 1) // BLOCK_SIZE
        ...
        slot_logical_block = pos // BLOCK_SIZE
        slot_offset = pos % BLOCK_SIZE
        slot_block = int(block_table[b * max_ctx_blocks + slot_logical_block].item())
